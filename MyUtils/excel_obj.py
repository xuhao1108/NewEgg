...
class ReadExcel(object):
    ...
    def read_all_data(self, row_header=True, col_header=False):
        ...
        """
        data = []
        rows = self.get_row_number()
        cols = self.get_col_number()
        if row_header:
...
class WriteExcel(object):
    def __init__(self, excel_path, save_path=None, sheet_name=None, always_save=True):
        """
        初始化
    ...
    def write_cell(self, data, row=0, col=0, cell_size=None, image_size=None, save_flag=True):
        ...
        if image_size:
            ...
            try:
                ...
                pass
        position = get_cell_position(row, col)
    ...
    def set_row_height(self, height, row=0, save_flag=True):
        ...
        :return:
        """
        self.sheet.row_dimensions[get_row_position(row)].height = height
        if self.always_save or save_flag:
            self.save_to_excel()
    ...
    def set_col_width(self, width, col=0, save_flag=True):
        ...
        :return:
        """
        self.sheet.column_dimensions[get_col_position(col)].width = width
        if self.always_save or save_flag:
            self.save_to_excel()
    ...
    def merge_cell(self, start_row, start_col, stop_row, stop_col, save_flag=True):
        ...
        :return:
        """
        self.sheet.merge_cells('{}:{}'.format(get_cell_position(start_row, start_col), get_cell_position(stop_row, stop_col)))
        if self.always_save or save_flag:
            self.save_to_excel()
